...
I recommend that you review those lectures, and code examples:

* [lecture 1](https://www.cs.dartmouth.edu/~tjp/cs10/notes8.html) (cached here as [PDF](cs10-tree1.pdf));
* [lecture 2](https://www.cs.dartmouth.edu/~tjp/cs10/notes9.html) (cached here as [PDF](cs10-tree2.pdf));
* [BinaryTree.java](cs10-BinaryTree.java);
* [BST.java](cs10-BST.java).

## Tree module

The ultimate tree module ([tree9](tree9)) has the following files:
...
* `diff89`, the output of `diff` showing how this version of the code differs from one of the precursors (tree8) below.
...
Try `cd ~cs50/examples; diff tree1/tree.c tree2/tree.c` to see what changed.
...
Review the [memory.c]({{site.examples}}/memory.c) module.

***Example: [tree9](tree9)***

tree9 finally has an interesting test program!
It reads stock quotes from Yahoo Finance and stores them in the binary tree.
...
It adds a target for `make test` to the makefile.
Try it!

***diffs***

Each folder includes a text file that shows a 'diff' with the previous example; it's worth noting how little code we needed to add to support `tree_delete()` in tree7, to track malloc/free in tree8, and no changes to tree.c to support a tree of stock quotes vs a tree of strings in tree9.
